...
class BertModel(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      adapter_module: (optional) whether or not to add the adaptive layers
      for the adaptive
...
def get_trainable_vars_from_ckpt(init_checkpoint):
  """try switching to trainable variables only"""
  vars_list = []
  for var in tf.trainable_variables(init_checkpoint):
    name = var[0]
...
def add_adapter_module(prev_output,
                       original_size,
                       bottleneck_size,
                       nonlinar_activation=gelu,
                       initializer_range=0.02):
  """Adapter Module for help with fine-tuning.

  This is an implementation of the method found here:
  https://arxiv.org/pdf/1902.00751.pdf

  Args:
    prev_output: Output of the layer immediately before this function is called.
    original_size: int. The size of the previous layer (where prev_output
    comes from).
    bottleneck_size: int. The size of the "bottleneck" (a.k.a., the feed
    forward down-project) layer.
    nonlinear_activation: function. The non-linear activation function to apply
      to the output of the bottlenecked layer.
    initializer_range: float. Range of the initializer (stddev of truncated
    normal).

  Returns:
    The output of the final layer of the adapter module
  """
  with tf.variable_scope("adapters"):
    print("Adding an adapter module...")

    # Run a feedforward down-project on the input layer
    ...
    # Run a feedforward up-project to create the final layer
    output = tf.layers.dense(
        output,
        original_size,
        kernel_initializer=tf.truncated_normal_initializer(stddev=1e-3))
    ...
    # Skip-connection
    output = layer_norm(output + prev_output)
...
def transformer_model(input_tensor,
  ...
  Also see: https://github.com/tensorflow/tensor2tensor/blob/master
  ...
  Args:
    ...
    do_return_all_layers: Whether to also return all layers or just the final
      layer.
    adapter_module: Whether to add the adapter module layers for transfer
    learning.
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.variable_scope("attention"):
        # NOTE: Beginning of multi-headed attention layer
        attention_heads = []
        ...
        # NOTE: Multi-headed attention projection layer
        ...
        with tf.variable_scope("output"):
          attention_output = tf.layers.dense(
              ...
              hidden_size,
              kernel_initializer=create_initializer(initializer_range))
          # Adapter module goes here; in between the feed-forward and the
          # dropout/normalization
          if adapter_module:
            attention_output = add_adapter_module(
                prev_output=attention_output,
                original_size=hidden_size,
                # was projected to this size in the previous layer
                bottleneck_size=64,
                initializer_range=initializer_range)
          attention_output = dropout(attention_output, hidden_dropout_prob)
          attention_output = layer_norm(attention_output + layer_input)

      # NOTE: Beginning of feed-forward layer
      ...
      # NOTE: Feed-forward projection layer
      ...
      with tf.variable_scope("output"):
        ...
        # dropout/normalization
        if adapter_module:
          layer_output = add_adapter_module(
