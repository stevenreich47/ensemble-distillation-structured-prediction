...
def load_vocab(vocab_file) -> Dict[str, int]:
  """Loads a vocabulary file into a dictionary."""
  vocab: collections.OrderedDict[str, int] = collections.OrderedDict()
  index = 0
  with tf.gfile.GFile(vocab_file, "r") as reader:
    while True:
      token: str = convert_to_unicode(reader.readline())
      if not token:
        break
      token = token.strip()
      vocab[token] = index
...
def convert_tokens_to_ids(vocab: Dict[str, int],
                          tokens: List[str]) -> List[int]:
  return [vocab[item] for item in tokens]


def convert_ids_to_tokens(inv_vocab: Dict[int, str], ids: List[int]) \
    -> List[str]:
  return [inv_vocab[item] for item in ids]
