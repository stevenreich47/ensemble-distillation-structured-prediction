...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
  ...
  memory efficient gradient implementation inspired by "Training Deep Nets
  with Sublinear Memory Cost"
  by Chen et al. 2016 (https://arxiv.org/abs/1604.06174)

  ys,xs,grad_ys,kwargs are the arguments to standard tensorflow tf.gradients
  (https://www.tensorflow.org/versions/r0.12/api_docs/python/train.html
  #gradients)

  'checkpoints' can either be
    - a list consisting of tensors from the forward pass of the neural net
      that we should re-use when calculating the gradients in the backward
      pass
      all other tensors that do not appear in this list will be re-computed
    - a string specifying how this list should be determined. currently we
      support
  ...
  if type(checkpoints) is not list:
    if checkpoints == 'collection':
      ...
      def fixdims(
          t):  # tf.Dimension values are not compatible with int, convert
        # manually
        try:
      ...
      ts_all = [t for t in ts_all if
                np.prod(fixdims(t.shape)) > MIN_CHECKPOINT_NODE_SIZE]
      ...
      for ts in [ts_filtered, ts_all]:
        ...
        for t in ts:
          b = set(
              ge.get_backward_walk_ops(t.op, inclusive=True,
                                       within_ops=fwd_ops))
          f = set(
              ge.get_forward_walk_ops(t.op, inclusive=False,
                                      within_ops=fwd_ops))
          # check that there are not shortcuts
          b_inp = set([inp for op in b for inp in op.inputs]).intersection(
              ts_all)
          f_inp = set([inp for op in f for inp in op.inputs]).intersection(
              ts_all)
          if not set(b_inp).intersection(f_inp) and len(b_inp) + len(
              f_inp) >= len(ts_all):
      ...
      if not bottleneck_ts:
        raise Exception(
            ...
            'nodes '
            'manually, or use checkpoints="speed".')
  ...
  for ts in checkpoints_sorted_lists[::-1]:
    ...
    checkpoints_disconnected_other = [checkpoints_disconnected[r] for r in
                                      checkpoints_other]
    ...
    copied_ops = info._transformed_ops.values()
    debug_print("Copied %s to %s", ops_to_copy, copied_ops)
    ge.reroute_ts(checkpoints_disconnected_other, checkpoints_other,
    ...
    def _unsparsify(x):
      if not isinstance(x, tf.IndexedSlices):
        return x
      assert x.dense_shape is not None, "memory_saving_gradients encountered " \
                                        "sparse gradients of unknown shape"
      indices = x.indices
...
def my_add_control_inputs(wait_to_do_ops, inputs_to_do_before):
  for op in wait_to_do_ops:
    ci = [i for i in inputs_to_do_before if
          op.control_inputs is None or i not in op.control_inputs]
